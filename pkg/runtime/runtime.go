...
func (a *DaprRuntime) beginPubSub(name string, ps pubsub.PubSub) error {
	...
	for topic, route := range v.routes {
		...
		if err := ps.Subscribe(pubsub.SubscribeRequest{
			...
			routePath, shouldProcess, err := findMatchingRoute(&route, cloudEvent)
			if err != nil {
				return err
			}
			if !shouldProcess {
...
func findMatchingRoute(route *Route, cloudEvent interface{}) (path string, shouldProcess bool, err error) {
	hasRules := len(route.routes) > 0
	if hasRules {
		data := map[string]interface{}{
			"event": cloudEvent,
		}
		rule, err := matchRoutingRule(route, data)
		if err != nil {
			return "", false, err
		}
		if rule != nil {
...
func matchRoutingRule(route *Route, data map[string]interface{}) (*runtime_pubsub.Route, error) {
	for _, rule := range route.routes {
		if rule.Match == nil {
			return rule, nil
		}
...
func (a *DaprRuntime) getTopicRoutes() (map[string]TopicRoute, error) {
	...
	for _, s := range subscriptions {
		...
		topicRoutes[s.PubsubName].routes[s.Topic] = Route{metadata: s.Metadata, routes: s.Routes}
	}
